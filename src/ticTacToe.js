//Basic HTML Elements
    ...
    function elementBuilder(elType, className, parent) {
      ...
      const body = document.getElementById("webgames");
      ...
      const playerOneScoreCount = elementBuilder(
        "div",
        "score-number",
        playerOneProfile
      );
      ...
      const playerTwoScoreCount = elementBuilder(
        "div",
        "score-number",
        playerTwoProfile
      );
      ...
      const gameBoard = (gameSpaces, parent, newflowArray, moveCount) => {
        ...
        const scoreKeeper = (winSet) => {
          ...
          if (winResult) {
            ...
            let winElement = document.getElementById(
              `${winningProfile.id}` + `-score`
            );
            ...
            let loseElement = document.getElementById(
              `${losingProfile.id}` + `-score`
            );
      ...
      const initialBoardObj = gameBoard(
        spaces,
        gameContainer,
        flowArray,
        initialMoveCount
      ...
      const win = (array, boardPiece) => {
        ...
        for (let i = 0; i < array.length; i++) {
          if (
            boardArray.includes(0) &&
            boardArray.includes(1) &&
            boardArray.includes(2)
          ) {
            winner = true;
            return { winner };
          } else if (
            boardArray.includes(3) &&
            boardArray.includes(4) &&
            boardArray.includes(5)
          ) {
            winner = true;
            return { winner };
          } else if (
      ...
      function winChecker(boardArray, moveCount) {
        ...
        if (scenarioX.winner) {
          ...
          let messageContainer = elementBuilder(
            "div",
            "message-container",
            mainContainer
          );
        ...
        if (
          scenarioX.winner === false &&
          scenarioO.winner === false &&
          moveCount.length === 9
        ) {
          let messageContainer = elementBuilder(
            "div",
